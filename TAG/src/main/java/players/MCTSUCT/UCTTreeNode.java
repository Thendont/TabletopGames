...
class UCTTreeNode {
    ...
    Map<AbstractAction, UCTTreeNode> children = new HashMap<>();
    // Depth of this node
    final int depth;
    ...
    protected UCTTreeNode(UCTPlayer player, UCTTreeNode parent, AbstractGameState state, Random rnd) {
        this.player = player;
        this.fmCallsCount = 0;
        this.parent = parent;
    ...
    void mctsSearch() {
        ...
        UCTParams params = player.getParameters();
        ...
        while (!stop) {
            ...
            UCTTreeNode selected = treePolicy();
     ...
     */
    private UCTTreeNode treePolicy() {

        UCTTreeNode cur = this;
    ...
    private UCTTreeNode expand() {
        ...
        UCTTreeNode tn = new UCTTreeNode(player, this, nextState, rnd);
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : children.keySet()) {
            UCTTreeNode child = children.get(action);
            if (child == null)
    ...
    AbstractAction bestAction() {
        ...
        for (AbstractAction action : children.keySet()) {
            if (children.get(action) != null) {
                UCTTreeNode node = children.get(action);
